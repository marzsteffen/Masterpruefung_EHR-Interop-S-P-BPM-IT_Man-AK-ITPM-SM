...
<xsl:stylesheet version="1.0"
                ...
                id="ELGA_Referenzstylesheet_1.11.1+20241006">
      ...
      function formatTable() {
        Array.from(document.querySelectorAll(".table_expand_collapse_control")).forEach((tableController) => {
          tableController.addEventListener("click", function () {
            addExpandCollapseListener(tableController);
          });
        });
      }

      function addExpandCollapseListener(controller) {
        const table = document.getElementById(controller.getAttribute("data-targetTable"));
        ...
        if(tableShouldBeExpanded) {
          controller.classList.remove("table_expand_collapse_control_expand");
          controller.classList.add("table_expand_collapse_control_collapse");
          controller.text = "weniger anzeigen";
        } else {
          controller.classList.remove("table_expand_collapse_control_collapse");
          controller.classList.add("table_expand_collapse_control_expand");
          controller.text = "alles anzeigen";
        }
      }
  ]]>
</xsl:variable>
...
<xsl:for-each select="/n1:ClinicalDocument/n1:author">
  ...
  <xsl:template name="getDocumentInformation">
    ...
    <p class="metaInformationLine">
      <xsl:text>Angezeigt mit ELGA_Referenzstylesheet_1.11.1+20241006</xsl:text>
    </p>
    <p class="metaInformationLine">
